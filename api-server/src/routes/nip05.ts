...
// ─── POST /nip05 — Create a NIP-05 entry ────────────────────────────────────
// Users can create for themselves, admins can create for any pubkey
router.post("/", requireAuth, async (req: Request, res: Response) => {
  ...
  if (cleanName !== name.trim().toLowerCase()) {
    res.status(400).json({ error: "Name can only contain lowercase letters, numbers, underscores, dots, and hyphens" });
    return;
  }
  ...
  // Check if name is already taken on this domain
  ...
  let targetPubkey = user.pubkey;

  if (rawPubkey && rawPubkey !== user.pubkey) {
    // Only admins can assign NIP-05 to other pubkeys
    if (!user.admin) {
      res.status(403).json({ error: "Only admins can assign NIP-05 to other users" });
      return;
    }
    ...
    targetPubkey = hexPubkey.toLowerCase();
  }

  // Limit: max 2 NIP-05 entries per pubkey on root domain
  const count = await prisma.nip05.count({ where: { pubkey: targetPubkey, domain } });
  if (count >= 2 && !user.admin) {
    res.status(400).json({ error: "Maximum 2 NIP-05 identities per user on this domain" });
    return;
  }

  // Create NIP-05 entry
  const nip05 = await prisma.nip05.create({
    data: { name: cleanName, domain, pubkey: targetPubkey },
  });
